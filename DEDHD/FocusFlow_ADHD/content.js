...
(function () {
    ...
    document.addEventListener('scroll', () => {
        const now = Date.now();
        const dy = Math.abs(window.scrollY - raw.lastScrollY);
        const dt = now - raw.lastScrollTime;
        const speed = dt > 0 ? dy / dt : 0; // px/ms
    ...
    const computeSignals = () => {
        ...
        signals.tabSwitching = tabs30 > 8 ? 3 : tabs30 > 4 ? 2 : tabs30 > 2 ? 1 : 0;
    ...
    // Reading guide line for readingSpeed lv3
    const getReadingGuide = () => document.getElementById('ff-reading-guide') || (() => {
        const el = document.createElement('div'); el.id = 'ff-reading-guide';
        el.style.cssText = 'position:fixed;left:0;right:0;height:2px;background:rgba(74,144,226,0.5);pointer-events:none;z-index:99995;display:none;';
        document.body.appendChild(el); return el;
    })();
    document.addEventListener('mousemove', (e) => {
        const guide = document.getElementById('ff-reading-guide');
        if (guide && guide.style.display !== 'none') guide.style.top = e.clientY + 'px';
    });
    ...
    // ── SCROLL THROTTLE — fights back against fast scrolling ─────────
    let scrollThrottleActive = false;
    let lastThrottledScroll = 0;

    const applyScrollThrottle = (active) => {
        if (active === scrollThrottleActive) return;
        scrollThrottleActive = active;

        if (active) {
            // Override scroll events to slow them down
            document.documentElement.style.scrollBehavior = 'smooth';
            window._ffScrollHandler = (e) => {
                const now = Date.now();
                const gap = now - lastThrottledScroll;
                if (gap < 120) { // allow one scroll per 120ms max
                    e.preventDefault();
                    e.stopPropagation();
                }
                lastThrottledScroll = now;
            };
            window.addEventListener('wheel', window._ffScrollHandler, { passive: false, capture: true });
        } else {
            document.documentElement.style.scrollBehavior = '';
            if (window._ffScrollHandler) {
                window.removeEventListener('wheel', window._ffScrollHandler, { capture: true });
                window._ffScrollHandler = null;
            }
        }
    };
    ...
    const applyIndividualEffects = () => {
        ...
        guide.style.display = signals.readingSpeed === 3 ? 'block' : 'none';
        ...
        // Scroll speed → throttle on lv2+, hard throttle on lv3
        applyScrollThrottle(signals.scrollSpeed >= 2);
        ...
        // Typing speed → screen shake at lv3
        document.body.classList.remove('ff-type-shake');
        if (signals.typingSpeed === 3) document.body.classList.add('ff-type-shake');
    ...
    const applyMasterStyles = () => {
        ...
        if (simplificationAllowed === true && masterLevel > 0) {
    ...
    const injectUI = () => {
        ...
        promptDiv.innerHTML = `
            <div class="ff-bubble-inner">
                <div class="ff-glitch-title" data-text="FOCUS ALERT">FOCUS ALERT</div>
                <p class="ff-bubble-msg">Neural fatigue detected.<br>Engage distraction shield?</p>
                <div class="ff-bubble-buttons">
                    <button id="ff-yes" class="ff-btn-yes">ENGAGE</button>
                    <button id="ff-no" class="ff-btn-no">DISMISS</button>
                </div>
        ...
        // Indicators
        if (!document.getElementById('ff-nav-line')) {
            const line = document.createElement('div'); line.id = 'ff-nav-line';
    ...
    const mainInterval = setInterval(async () => {
        ...
        if (masterLevel > 0 && simplificationAllowed === null && prompt) {
        ...
        // Update nav line and dot color
        const line = document.getElementById('ff-nav-line');
        const dot = document.getElementById('ff-status-dot');
        const colors = ['transparent', '#00FF88', '#FFA500', '#FF4D4D'];
        if (line) line.style.background = colors[masterLevel] || 'transparent';
